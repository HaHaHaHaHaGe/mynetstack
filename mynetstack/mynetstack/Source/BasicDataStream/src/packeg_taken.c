...
u8 search_packeg(u8*src1, u8*src2, u32 len1, u32 len2, u8**rdata_1,u8**rdata_2,u32*rlen_1,u32*rlen_2)
{
			...
			if (n == sizeof(packet_header))
			{
				if (src_1 + 4 > src1 + len1)
				{
					...
					if (len > __REDUCTION_DATA_LENGTH__)
						return NO;
					crc32_1 = *(src2 + (src_1 + 4 - src1 - len1));
#ifdef Little_Endian
	...
	if (len2 >= 3 + 8)
		if ((*(src1 - 1) == packet_header[0]) && (*(src2) == packet_header[1]) && (*(src2 + 1) == packet_header[2]) && (*(src2 + 2) == packet_header[3]))
			src_2 = src2 + 3;

	if (len2 >= 2 + 8)
		if ((*(src1 - 1) == packet_header[1]) && (*(src1 - 2) == packet_header[0]) && (*(src2) == packet_header[2]) && (*(src2 + 1) == packet_header[3]))
			src_2 = src2 + 2;

	if (len2 >= 1 + 8)
		if ((*(src1 - 1) == packet_header[2]) && (*(src1 - 2) == packet_header[1]) && (*(src1 - 3) == packet_header[0]) && (*(src2) == packet_header[3]))
			src_2 = src2 + 1;
